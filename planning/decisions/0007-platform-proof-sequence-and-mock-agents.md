...
6. **Canonical GitHub validation** — the deliberately triggered self-hosted runner verifies the frozen merge candidate.
...
Visual-regression baselines should be generated and compared in a stable environment. The self-hosted runner is the canonical environment for accepted baselines because font and rendering differences can make cross-environment screenshots noisy. Assistant-local screenshots remain useful development evidence and design-review material.
...
The first response to artifact pressure is failure-only uploads, compression, short retention, and deletion of obsolete workflow runs. A local archive on the self-hosted runner is permitted later only if this policy remains insufficient.

A future local archive must live outside the ephemeral Actions workspace, enforce a total size ceiling, and rotate by age and count. It is not required for the current documentation or canonical workflow.
...
- The self-hosted runner remains a canonical gate rather than an ordinary development executor.
