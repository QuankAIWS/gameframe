...
Scribbles GameFrame benefits from public GitHub-hosted Actions capacity, public inspection, easier sharing, and the possibility of later community interest. The project owner nevertheless wants to retain maximum control over original code while the platform remains experimental and its future licensing model is unsettled.
...
Public repositories also create a material security boundary around CI. Untrusted fork code must not be executed on the private self-hosted runner.

## Decision

`QuankAIWS/scribbles-gameframe` will be prepared for public visibility while remaining proprietary and all rights reserved.
...
The existing private self-hosted runner must not execute public GameFrame branches or pull-request code.
...
The repository must not be made public until all of the following are complete:
...
4. Public workflows no longer target the private self-hosted runner.
...
- The owner retains copyright and grants no general reuse license.
- Public GitHub-hosted validation can replace private runner consumption for this repository.
