...
12. [`monster-master-rpg-current-creative-direction.md`](monster-master-rpg-current-creative-direction.md) — controlling current-phase direction: several starter possibilities, adult-world posture, broad tonal range, plot-agent package versus DM realization, the Master Baiter concept, and manual asset production.
...
15. [`monster-master-rpg-asset-register.md`](monster-master-rpg-asset-register.md) — shared starter-foundation priorities, family extensions, source-master lifecycle, attempt tracking, reuse audit, and acceptance evidence.
16. [`assets/monster-master-rpg-asset-register.json`](assets/monster-master-rpg-asset-register.json) — machine-readable reuse, shared P0, selected-family P1, and deferred asset identities; no scenario package owns the register.
...
Specific Monster Master catalog contents, selected causes, actor secrets, clue answers, event eligibility, pressure state, and active packages belong in the private RPG GM Runtime repository.

The runtime currently stores the curated catalog, one explicitly non-canonical deterministic package, and intro implementation notes in:
...
GameFrame owns public templates, lore direction, presentation semantics, semantic asset roles, and the reusable production register. It must not become a second store for active hidden package truth.
...
2. commit one selected package per run before meaningful investigation;
3. let the DM model realize that package dynamically around player action;
4. create, edit, clean, and integrate a shared starter asset foundation;
5. add selected family extensions after the foundation is visually proven;
6. validate that a deterministic CI package proves contracts without becoming the default player story;
7. use what is learned to refine the future plot-agent template.
...
Repository-internal implementation notes, private runtime prompts, provider credentials, selected packages, clue answers, and temporary branch plans are not shared-document candidates.
