...
test("the Games catalog opens Role-Playing Games, Battle Simulator, Casual Games, and standalone game surfaces", async ({ page }) => {
  ...
  await expect(page.locator("#gameframe-destination-bar")).toBeVisible();
  await expect(page.locator("[data-gameframe-destination-title]")).toHaveText("CASUAL GAMES");
  await expect(page.locator("#gameframe-destination-bar [data-gameframe-games]")).toHaveClass(/is-active/);
  ...
  const cascadeLauncher = page.getByRole("link", { name: "Open Cascade Crush", exact: true });
  await expect(cascadeLauncher).toHaveAttribute("href", "/cascade.html");
  await expect(cascadeLauncher).toContainText("300 levels");
  await expect(cascadeLauncher.locator(".cascade-preview-board i")).toHaveCount(48);
  await cascadeLauncher.click();
