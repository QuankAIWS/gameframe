...
async function runChainedStripesWithMobileViewportPulses(page) {
...
test("Cascade mobile chained stripes survive browser-chrome viewport changes without black flash or reload", async ({ context, page }, testInfo) => {
  ...
  await runChainedStripesWithMobileViewportPulses(page);
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      visibleEffectGroups: [...document.querySelectorAll(".cascade-juice-layer > *")]
  ...
  expect(result.peakParticles).toBeGreaterThan(100);
