...
async function start() {
  ...
  return { server, directory, baseUrl: `http://127.0.0.1:${address.port}` };
}
...
test("only Runtime can move Pell for inspection and the durable transform appears on later player attach", async () => {
  ...
  try {
    ...
    const playerAttempt = await fetch(path, {
      method: "POST",
      headers: { "content-type": "application/json", "x-gameframe-player-id": "player:ada" },
      body: JSON.stringify(actorRequest()),
    });
    assert.equal(playerAttempt.status, 403);

    const serviceHeaders = { "content-type": "application/json", "x-gameframe-service-id": "rpg-gm-runtime" };
    const inspectedResponse = await fetch(path, { method: "POST", headers: serviceHeaders, body: JSON.stringify(actorRequest()) });
    ...
    const retry = await fetch(path, { method: "POST", headers: serviceHeaders, body: JSON.stringify(actorRequest()) }).then((response) => response.json()) as any;
    ...
    const playerAttach = await fetch(`${running.baseUrl}/api/rpg/campaigns/campaign-monster-master-reference/exploration/attach`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-gameframe-player-id": "player:ada" },
      body: JSON.stringify({ protocolVersion: 1, kind: "campaign.exploration.attach", campaignId: "campaign-monster-master-reference" }),
    });
    assert.equal(playerAttach.status, 200);
    const materialized = await playerAttach.json() as any;
    const pell = materialized.materialization.anchors.find((anchor: any) => anchor.semanticId === "npc.warden-pell");
    const mara = materialized.materialization.anchors.find((anchor: any) => anchor.semanticId === "npc.mara-venn");
    assert.deepEqual({ x: pell.x, y: pell.y }, { x: inspected.transform.x, y: inspected.transform.y });
...
test("Runtime cannot reuse an actor operation id for a different target", async () => {
  ...
  try {
    const path = `${running.baseUrl}/api/rpg/campaigns/campaign-monster-master-reference/exploration/actor-inspect`;
    const headers = { "content-type": "application/json", "x-gameframe-service-id": "rpg-gm-runtime" };
    const first = await fetch(path, { method: "POST", headers, body: JSON.stringify(actorRequest()) });
    assert.equal(first.status, 200);
    const conflicting = await fetch(path, { method: "POST", headers, body: JSON.stringify(actorRequest({ targetEntityId: "npc.warden-pell" })) });
