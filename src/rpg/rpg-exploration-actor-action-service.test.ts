...
function store() {
  ...
  return new SqliteRpgExplorationActorPositionStore({
...
test("Pell approaches Mara through legal GameFrame cells without crossing occupied cells", () => {
  ...
  try {
    ...
    // Put the durable player on a plausible shortest-route cell. NPC pathfinding
    // must treat the player's actual position as occupied and route around it.
    service.applyPlayerPosition(
      materialization,
      "trainer:_RmGMYy8smZq7uqHUwdOgs",
      { x: 8, y: 7 },
    );
    ...
    assert.deepEqual(
      ...
      { x: receipt.transform.x, y: receipt.transform.y },
    );
  } finally {
    positions.close();
  }
...
test("exact operation retry is idempotent and changed reuse fails closed", () => {
  ...
  try {
    ...
    assert.throws(
      () => service.inspect(
        request({ targetEntityId: "npc.warden-pell" }),
...
test("actor transforms are scoped per scene instead of overwriting other scene positions", () => {
  ...
  try {
    const checkpointRef = {
      materializationId: "rpg-exploration:checkpoint",
      version: "1",
      hash: "hash-checkpoint",
    };
    const woodsRef = {
      materializationId: "rpg-exploration:woods",
      version: "1",
      hash: "hash-woods",
    };
    positions.commit({
    ...
    assert.equal(
      positions.read("campaign-one", "scene.crooked-checkpoint", "npc.warden-pell")?.transform.x,
      7,
    );
    assert.equal(
      positions.read("campaign-one", "scene.west-woods", "npc.warden-pell")?.transform.x,
      12,
    );
  } finally {
    positions.close();
