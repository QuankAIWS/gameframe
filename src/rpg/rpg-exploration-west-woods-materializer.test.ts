...
test("Crooked Checkpoint route is also physically authorizable through the shared profile", () => {
  const projection = JSON.parse(readFileSync(fixturePath, "utf8")).projection;
  const materialization = materializeRpgExplorationProjection(projection);
  const route = materialization.anchors.find((anchor) =>
    anchor.semanticId === "route.crooked-checkpoint-west-woods"
  );
  assert.equal(route?.interactionTargetId, "route:route.crooked-checkpoint-west-woods");
});
