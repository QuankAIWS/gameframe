...
function flipsFor(
  ...
  for (const [dr, dc] of DIRECTIONS) {
    ...
    if (
      line.length > 0
      && inside(currentRow, currentColumn)
      && board[currentRow][currentColumn] === disc
    ) {
      flips.push(...line);
    }
  }
...
function legalActions(board: OthelloCell[][], disc: OthelloDisc): OthelloAction[] {
  ...
  for (let row = 0; row < SIZE; row += 1) {
    for (let column = 0; column < SIZE; column += 1) {
      if (flipsFor(board, disc, row, column).length > 0) {
        result.push({ type: "place", row, column });
      }
    }
  }
  return result;
...
export const othelloDefinition: GameDefinition<OthelloState, OthelloAction, OthelloObservation> = {
  ...
  getStatus(state) {
    ...
    return {
      lifecycle: "completed",
      winnerPlayerId: scores.dark > scores.light ? state.players.dark : state.players.light,
      draw: false,
    };
  },
  ...
  applyAction(state, playerId, action) {
    ...
    return {
      state: {
        ...
        move: state.move + 1,
        complete,
        lastMove: [action.row, action.column],
      },
