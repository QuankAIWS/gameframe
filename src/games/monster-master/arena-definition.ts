...
export const MONSTER_MASTER_ARENA_RULES_VERSION = 2;
...
function secondEmberling(
  playerIndex: number,
  source: MonsterMasterUnit,
): MonsterMasterUnit {
  const unit = cloneMonsterMasterUnit(source);
  unit.id = `${arenaPrefix(playerIndex)}-emberling-2`;
  unit.tags = [...(unit.tags ?? []), ARENA_UNIT_TAG, "arena-monster-slot-3"];
  return unit;
}
...
export function createMonsterMasterArenaState(playerIds: readonly PlayerId[]): MonsterMasterState {
  ...
  state.playerIds.forEach((playerId, playerIndex) => {
    ...
    if (!emberling) throw new Error("Monster Master Arena requires an Emberling prototype unit.");
    const extra = secondEmberling(playerIndex, emberling);
    roster.push(extra);
