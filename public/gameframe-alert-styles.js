...
.gameframe-session-badge .gameframe-alerts{position:static;order:-1;flex:0 0 auto;pointer-events:auto}
.gameframe-session-badge .gameframe-alerts-trigger{position:relative;display:grid;place-items:center;width:40px;height:40px;padding:0;border:1px solid rgba(174,183,255,.4);border-radius:50%;background:rgba(12,17,34,.96);color:#eef0ff;cursor:pointer}
...
@media(max-width:720px){.gameframe-session-badge:has(.gameframe-alerts)>span:not(.gameframe-session-avatar){display:none}.gameframe-session-badge .gameframe-alerts-trigger{display:grid!important;width:36px;height:36px}.gameframe-alerts-panel{position:fixed;top:69px;right:8px;width:calc(100vw - 16px)}}`;
