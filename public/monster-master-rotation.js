...
const CAMERA_STORAGE_KEY = "gameframe:monster-master:camera-quarter";
const CORNER_NAMES = ["Northwest", "Northeast", "Southeast", "Southwest"];
const camera = {
  x: 11.5,
  y: 11.5,
  zoom: 1,
...
camera.angle = camera.quarter * Math.PI / 2;

const clamp = (value, min, max) => Math.min(Math.max(value, min), max);
const key = ({ x, y }) => `${x},${y}`;
const normalizeQuarter = (value) => ((Math.round(value) % 4) + 4) % 4;
...
function loadQuarter() {
  try { return normalizeQuarter(Number(localStorage.getItem(CAMERA_STORAGE_KEY) ?? 0)); }
  catch { return 0; }
}
function storeQuarter() {
  try { localStorage.setItem(CAMERA_STORAGE_KEY, String(camera.quarter)); }
  catch { /* Camera preference persistence is optional. */ }
}
...
function rotateDelta(dx, dy, angle = camera.angle) {
  const cosine = Math.cos(angle);
  const sine = Math.sin(angle);
  return { x: dx * cosine - dy * sine, y: dx * sine + dy * cosine };
}
function unrotateDelta(x, y, angle = camera.angle) {
  const cosine = Math.cos(angle);
  const sine = Math.sin(angle);
  return { x: x * cosine + y * sine, y: -x * sine + y * cosine };
}
function cameraSpace(coordinate) {
  return rotateDelta(coordinate.x - camera.x, coordinate.y - camera.y);
}
function depthOf(coordinate) {
  const rotated = cameraSpace(coordinate);
  return rotated.x + rotated.y;
}
function crossDepthOf(coordinate) {
  return cameraSpace(coordinate).x;
}
function worldToScreen(coordinate, frame = layout(), z = 0) {
  if (!frame) return { x: 0, y: 0 };
  const rotated = cameraSpace(coordinate);
  return {
    x: frame.originX + (rotated.x - rotated.y) * frame.halfWidth,
    y: frame.originY + (rotated.x + rotated.y) * frame.halfHeight - z * frame.tileHeight * 0.88,
  };
}
function screenToWorld(point, frame = layout()) {
  if (!frame) return { x: 0, y: 0 };
  const projectedX = (point.x - frame.originX) / frame.halfWidth;
  const projectedY = (point.y - frame.originY) / frame.halfHeight;
  const rotated = { x: (projectedX + projectedY) / 2, y: (projectedY - projectedX) / 2 };
  const world = unrotateDelta(rotated.x, rotated.y);
  return { x: camera.x + world.x, y: camera.y + world.y };
}
function screenToTile(point, frame = layout(), candidate = view) {
  const map = mapOf(candidate);
  if (!frame || !map) return null;

  for (const unit of [...unitsOf(candidate)].sort((a, b) => depthOf(b.position) - depthOf(a.position))) {
...
function wallOpacity(candidate, coordinate, frame, state) {
  const wallPoint = worldToScreen(coordinate, frame);
  const wallDepth = depthOf(coordinate);
  const priorityIds = new Set([candidate.observation.activeUnitId, state.selectedUnitId].filter(Boolean));
  const orderedUnits = [...unitsOf(candidate)].sort((a, b) => Number(priorityIds.has(b.id)) - Number(priorityIds.has(a.id)));
  for (const unit of orderedUnits) {
    if (wallDepth <= depthOf(unit.position) + 0.05) continue;
    const unitPoint = worldToScreen(unit.position, frame);
    const horizontal = Math.abs(wallPoint.x - unitPoint.x);
    const vertical = unitPoint.y - wallPoint.y;
    if (horizontal <= frame.tileWidth * 0.62 && vertical >= -frame.tileHeight * 1.25 && vertical <= frame.tileHeight * 1.18) {
      return priorityIds.has(unit.id) ? 0.2 : 0.36;
    }
  }
  return 1;
}

function drawCell(context, candidate, coordinate, cell, frame, highlights, state) {
  ...
  const opacity = wallHeight ? wallOpacity(candidate, coordinate, frame, state) : 1;
  ...
  context.fill();
  context.strokeStyle = opacity < 1 ? "rgba(236,247,255,.72)" : "rgba(190,215,248,.18)";
  context.lineWidth = opacity < 1 ? 1.5 : 1;
  if (opacity < 1) context.setLineDash([5, 4]);
  context.stroke();
  context.setLineDash([]);
  context.restore();
  ...
  if (cell.terrain === "difficult" && !wallHeight) {
    ...
    for (const offset of [-0.2, 0, 0.2]) {
      context.beginPath();
      context.moveTo(point.x + frame.tileWidth * offset, point.y + frame.tileHeight * 0.14);
      context.lineTo(point.x + frame.tileWidth * (offset - 0.03), point.y - frame.tileHeight * 0.18);
      context.stroke();
    }
  }
}
...
function drawUnit(context, candidate, unit, position, frame, lift = 0) {
  ...
  const scale = clamp(frame.tileWidth / 76, 0.72, 1.45);
  const y = point.y - frame.tileHeight * (0.34 + lift * 0.25);
  ...
  if (unit.role === "master") {
    ...
    context.moveTo(0, -43);
    context.bezierCurveTo(20, -22, 18, 2, 0, 22);
    context.bezierCurveTo(-18, 2, -20, -22, 0, -43);
    context.closePath();
  } else {
  ...
  context.beginPath();
  context.arc(0, -34, 9, 0, Math.PI * 2);
  context.fill();
  context.restore();
  ...
  if (unit.id === candidate.observation.activeUnitId || unit.id === diagnostics().selectedUnitId) {
    ...
    context.lineWidth = Math.max(2, frame.tileWidth * 0.035);
    context.beginPath();
    context.ellipse(point.x, point.y, frame.tileWidth * 0.34, frame.tileHeight * 0.22, 0, 0, Math.PI * 2);
    context.stroke();
  }
  const health = clamp(unit.health / unit.maxHealth, 0, 1);
  const barWidth = clamp(frame.tileWidth * 0.68, 34, 88);
  const barY = y - clamp(frame.tileHeight * 1.55, 46, 78);
  context.fillStyle = "rgba(3,7,13,.86)";
  context.fillRect(point.x - barWidth / 2, barY, barWidth, 6);
  context.fillStyle = health > 0.45 ? "#6ee0a5" : "#ff9671";
  context.fillRect(point.x - barWidth / 2, barY, barWidth * health, 6);
}
...
function drawPreview(context, candidate, frame, state) {
  ...
  if (action.type === "move") {
    ...
    context.lineWidth = Math.max(3, frame.tileWidth * 0.055);
    context.lineCap = "round";
    context.lineJoin = "round";
    context.beginPath();
    [action.from, ...action.path].forEach((coordinate, index) => {
      const point = worldToScreen(coordinate, frame);
      if (index) context.lineTo(point.x, point.y - frame.tileHeight * 0.12);
      else context.moveTo(point.x, point.y - frame.tileHeight * 0.12);
    });
    context.stroke();
    ...
    context.lineWidth = Math.max(3, frame.tileWidth * 0.045);
    context.setLineDash([8, 6]);
    context.beginPath();
    context.moveTo(from.x, from.y - frame.tileHeight * 0.7);
    context.lineTo(target.x, target.y - frame.tileHeight * 0.7);
    context.stroke();
    context.setLineDash([]);
  }
}

function sceneEntries(candidate, map, frame, moving) {
  const entries = [];
  for (let y = 0; y < map.height; y += 1) {
    for (let x = 0; x < map.width; x += 1) {
      ...
      if (point.x > -frame.tileWidth * 2 && point.x < frame.width + frame.tileWidth * 2
        ...
        entries.push({ type: "cell", coordinate, cell: map.cells[y * map.width + x], depth: depthOf(coordinate), cross: crossDepthOf(coordinate) });
      }
    }
  }
  for (const unit of unitsOf(candidate)) {
    if (unit.id === moving?.unit.id) continue;
    entries.push({ type: "unit", unit, position: unit.position, lift: 0, depth: depthOf(unit.position) + 0.34, cross: crossDepthOf(unit.position) });
  }
  if (moving) {
    entries.push({ type: "unit", ...moving, depth: depthOf(moving.position) + 0.34, cross: crossDepthOf(moving.position) });
  }
  entries.sort((a, b) => a.depth - b.depth || a.cross - b.cross || (a.type === "cell" ? -1 : 1));
  return entries;
}

function updateCameraUi() {
  if (!ui) return;
  const corner = CORNER_NAMES[camera.quarter];
  ui.frame.dataset.cameraQuarter = String(camera.quarter);
  ui.frame.dataset.cameraCorner = corner.toLowerCase();
  ui.frame.dataset.rotating = String(rotating);
  ui.overlay.dataset.cameraQuarter = String(camera.quarter);
  ui.overlay.dataset.billboard = "camera-facing";
  if (ui.orientationLabel) ui.orientationLabel.textContent = corner;
  if (ui.rotateLeft) ui.rotateLeft.setAttribute("aria-label", `Rotate view left from ${corner}`);
  if (ui.rotateRight) ui.rotateRight.setAttribute("aria-label", `Rotate view right from ${corner}`);
}
...
function render({ candidate = view, moving = null } = {}) {
  ...
  gradient.addColorStop(0, "#10192a");
  gradient.addColorStop(0.58, "#080e19");
  gradient.addColorStop(1, "#050810");
  context.fillStyle = gradient;
  context.fillRect(0, 0, frame.width, frame.height);

  for (const entry of sceneEntries(candidate, map, frame, moving)) {
    if (entry.type === "cell") drawCell(context, candidate, entry.coordinate, entry.cell, frame, highlights, state);
    else drawUnit(context, candidate, entry.unit, entry.position, frame, entry.lift);
  }
  drawPreview(context, candidate, frame, state);

  ui.overlay.dataset.projection = "rotatable-dimetric";
  ui.overlay.dataset.actionable = String(interaction(hover, candidate, state) !== "none");
  ui.primary.dataset.projection = "rotatable-dimetric";
  updateCameraUi();
}
...
async function animateMove(candidate, effect) {
  ...
  try {
    ...
    ui.frame.dataset.animating = "false";
    render();
  }
}
async function runMoveQueue() {
  ...
  try {
    while (moveQueue.length) {
      ...
      await animateMove(next.candidate, next.effect);
    }
  } finally {
    animating = false;
    scheduleRender();
...
function clickControl(selector) {
  ...
  try { button.click(); }
  finally { internalCameraSync = false; }
}
...
function squareHit(point) {
  ...
  const coordinate = {
    x: bounds.x + Math.floor((point.x - originX) / size),
    y: bounds.y + Math.floor((point.y - originY) / size),
  };
  if (!visible(coordinate, bounds)) return null;
  const center = {
    x: originX + (coordinate.x - bounds.x + 0.5) * size,
    y: originY + (coordinate.y - bounds.y + 0.5) * size,
  };
...
function dispatchCoordinate(coordinate) {
  ...
  try {
    ui.primary.dispatchEvent(new MouseEvent("click", {
      ...
      cancelable: true,
      button: 0,
      clientX: rect.left + originX + (coordinate.x - bounds.x + 0.5) * size,
      clientY: rect.top + originY + (coordinate.y - bounds.y + 0.5) * size,
    }));
  } finally {
...
function shortestQuarterAngle(fromQuarter, toQuarter) {
  let difference = normalizeQuarter(toQuarter) - normalizeQuarter(fromQuarter);
  if (difference > 2) difference -= 4;
  if (difference < -2) difference += 4;
  return difference * Math.PI / 2;
}
function rotateTo(nextQuarter, { animate = true } = {}) {
  const normalized = normalizeQuarter(nextQuarter);
  if (rotating || normalized === camera.quarter) return;
  const fromQuarter = camera.quarter;
  const startAngle = camera.angle;
  const delta = shortestQuarterAngle(fromQuarter, normalized);
  const finish = () => {
    camera.quarter = normalized;
    camera.angle = normalized * Math.PI / 2;
    rotating = false;
    storeQuarter();
    updateCameraUi();
    render();
    ui.primary.dispatchEvent(new CustomEvent("gameframe:monster-camera-rotated", {
      bubbles: true,
      detail: { quarter: camera.quarter, corner: CORNER_NAMES[camera.quarter] },
    }));
  };
  ...
  const step = (now) => {
    ...
    camera.angle = startAngle + delta * eased;
    render();
    if (progress < 1) requestAnimationFrame(step);
    else finish();
  };
...
function pointerMove(event) {
  ...
  if (dragging?.id === event.pointerId && dragging.canDrag) {
    ...
    if (dragging.moved) {
      const frame = layout();
      const projected = {
        x: -(dx / frame.halfWidth + dy / frame.halfHeight) / 2,
        y: -(dy / frame.halfHeight - dx / frame.halfWidth) / 2,
      };
      const world = unrotateDelta(projected.x, projected.y);
      camera.x = dragging.startX + world.x;
      camera.y = dragging.startY + world.y;
      clampCamera();
      scheduleRender();
      event.preventDefault();
      event.stopImmediatePropagation();
...
function wheel(event) {
  ...
  event.preventDefault();
  event.stopImmediatePropagation();
  zoomAt(camera.zoom * Math.exp(-event.deltaY * 0.0016), localPoint(event));
}

function createRotationControls() {
  const controls = document.querySelector(".tactical-controls");
  if (!controls || controls.querySelector("#monster-master-rotate-left")) return;
  ...
  left.type = "button";
  left.id = "monster-master-rotate-left";
  left.className = "monster-master-rotate-button";
  left.textContent = "↶";

  const label = document.createElement("output");
  label.id = "monster-master-camera-corner";
  label.className = "monster-master-camera-corner";
  label.setAttribute("aria-live", "polite");

  const right = document.createElement("button");
  right.type = "button";
  right.id = "monster-master-rotate-right";
  right.className = "monster-master-rotate-button";
  right.textContent = "↷";

  group.append(left, label, right);
  controls.insertAdjacentElement("afterend", group);
  ui.rotateLeft = left;
  ui.rotateRight = right;
  ui.orientationLabel = label;
  left.addEventListener("click", () => rotateBy(-1));
  right.addEventListener("click", () => rotateBy(1));
  updateCameraUi();
}
function bindControls() {
  ...
  document.querySelector("#monster-master-center-field")?.addEventListener("click", () => {
    if (!internalCameraSync) centerField();
  });
  document.querySelector("#monster-master-center-active")?.addEventListener("click", () => {
    if (!internalCameraSync) centerOn(activeUnit()?.position);
  });
  document.querySelector("#monster-master-zoom-in")?.addEventListener("click", () => {
    if (!internalCameraSync) zoomAt(camera.zoom + 0.25, { x: layout().originX, y: layout().originY });
  });
  document.querySelector("#monster-master-zoom-out")?.addEventListener("click", () => {
    if (!internalCameraSync) zoomAt(camera.zoom - 0.25, { x: layout().originX, y: layout().originY });
  });
...
function bindKeyboard() {
  ...
  ui.primary.addEventListener("keydown", (event) => {
    ...
    else if (event.key === "+" || event.key === "=") zoomAt(camera.zoom + 0.25, { x: layout().originX, y: layout().originY });
    else if (event.key === "-" || event.key === "_") zoomAt(camera.zoom - 0.25, { x: layout().originX, y: layout().originY });
    else if (event.key === "[") rotateBy(-1);
    else if (event.key === "]") rotateBy(1);
...
function initialize() {
  ...
  ui = { frame, primary, overlay, rotateLeft: null, rotateRight: null, orientationLabel: null };
  frame.dataset.projectionReady = "true";
  frame.dataset.dragging = "false";
  frame.dataset.rotating = "false";
  primary.dataset.projection = "rotatable-dimetric";
  overlay.dataset.projection = "rotatable-dimetric";
  overlay.dataset.billboard = "camera-facing";
  ...
  primary.addEventListener("pointerleave", (event) => {
    if (!dragging?.moved) {
      hover = null;
      scheduleRender();
    }
  ...
  primary.addEventListener("dblclick", (event) => {
    event.preventDefault();
    event.stopImmediatePropagation();
    centerOn(activeUnit()?.position);
  }, true);
...
window.gameFrameMonsterProjection = {
  getCamera: () => ({
    ...
    corner: CORNER_NAMES[camera.quarter],
    angle: camera.angle,
  }),
  ...
  setRotation: (quarter, options) => rotateTo(quarter, options),
  render,
};
