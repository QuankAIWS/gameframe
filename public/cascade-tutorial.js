...
function visualMarkup(kind) {
  const tile = '<i class="cascade-tutorial-mini-tile" aria-hidden="true"></i>';
  if (kind === "match") return `${tile}${tile}${tile}`;
  if (kind === "stripe" || kind === "bomb" || kind === "color" || kind === "ice" || kind === "layered-ice") return tile;
  if (kind === "combo") return `${tile}${tile}`;
  if (kind === "collect") return `${tile}${tile}<strong>COLLECT</strong>`;
  if (kind === "hammer") return "🔨";
  if (kind === "weekly") return "<strong>30</strong><small>SECONDS</small>";
  return tile;
}
